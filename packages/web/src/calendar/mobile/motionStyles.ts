 ...
 * defined in motion.css (or controls.css) written directly into a .module.css rule silently
 * resolves to a hashed name with no matching @keyframes, and the animation never plays.
 * Inline style values aren't touched by that transform, so the keyframe name stays literal
 * here and actually resolves.
 */
export const slideUpEnter: CSSProperties = { animation: 'zpm-up 280ms var(--zp-ease)' };
export const fadeEnter: CSSProperties = { animation: 'zpm-fade 180ms var(--zp-ease)' };
