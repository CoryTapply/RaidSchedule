...
CSS Modules, no framework. `packages/web/src/design-system/zerpy/` is the vendored design system — `tokens/*.css`, `components/controls.css` and `styles.css` are copied verbatim from `design/UpdatedClaudeDesign/ClaudeDesignOutput/_ds/zerpy-design-system-.../`; re-copy them from there if the source sheet changes rather than hand-editing the vendored copies. `design-system/zerpy/components/*.tsx` (Button, IconButton, Field, Input, Select, SegmentedControl, Badge, Dialog, PageShell, Panel) are **hand-built**, not vendored — the exported design-system folder only ships a compiled runtime bundle for its own design-tool viewer, not importable component source (see `MIGRATION.md`'s "Component vendoring" section for the full explanation). If a real `.tsx` source ever becomes available, prefer it over the hand-built version, but keep the same public prop shape these currently expose. The per-class event card colors (`eventCard.module.css`) pin `color-mix(in srgb, …)` as static CSS specifically to prevent the OKLCH regression `design/README.md` warns about (only the raw class hex varies, via the `--class-color` custom property) — don't move that logic into JS-generated inline styles.
